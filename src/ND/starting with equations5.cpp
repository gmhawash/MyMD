...
int qbit,i1,i2,i3,i4,i5,x,xx,y,b,bb,gatecount, column,w, row,couont;
  int main()
  {
    y=20;
    i3=0;
...
ofstream myfile;
 ...
 cout << "input orderings generated" << "\n";;
 system("PAUSE");     
...
cout << "input vector :";
for ( i1 = 0; i1 < 20 ;++ i1 ){cout << array[row][i1]<<" ";}
cout << "\n";
system("PAUSE");


cout << "output vector:";
for ( i1 = 0; i1 < 20 ;++ i1 ){cout << outarray[i1]<<" ";}
cout << "\n";
system("PAUSE");
w=0;
for ( i1 = 0; i1 < 20 ;++ i1 )
{
     for (i4 = 0; i4 < gatecount+1 ;++ i4 )
     {
//    cout <<"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n";
         for (i5 = 0; i5 < 20 ;++ i5 ){if (control[i4][i5]==1 && outarray[i5]==0){w=1;break;}}    
         if (w==0 && outarray[i1]==0){outarray[i1]=1;}
         else if (w==0 && outarray[i1]==1){outarray[i1]=0;} 
     }
}
...
if (outarray [i1] != array[row][i1]){dif[i1]=1; cout << "input" << i1 <<" != " << "out" << i1;}    //registering if output does not equal input
}

for ( i1 = 0; i1 < 20 ;++ i1 )
{
if (dif[i1]==1 && outarray[i1]==0) {gatecount=gatecount+1;flip[gatecount]=i1; for (i3 = 0; i3 < 20 ;++ i3) {control[gatecount][i3]=cont[i3];}cont[i1]=1;cout <<"\n"<< dif[i1]<<" "<<outarray[i1]<<" x"<<"loop= "<< i1<<"\n";}
}

for ( i1 = 0; i1 < 20 ;++ i1 )
{   
if (dif[i1]==1 && outarray[i1]==1) {cont[i1]=0; gatecount=gatecount+1;flip[gatecount]=i1;for (i3 = 0; i3 < 20 ;++ i3) {control[gatecount][i3]=cont[i3];}cout <<"\n"<< "y"<<"\n";}            
}
