...
int qbit,i1,i2,i3,i4,i5,x,xx,y,b,bb,gatecount, column, row,couont, numberofcontrols,lastcontrol, repeat;
  int main()
  {
    y=20;
    repeat=0;
...
{ 
dif[i1]=0;    
if (outarray [i1]==1) {cont [i1]=1;numberofcontrols=numberofcontrols+1;lastcontrol=i1} else {cont [i1]=0;}            //registering all control lines
if (outarray [i1] != array[row][i1]){dif[i1]=1; cout << "input" << i1 <<" != " << "out" << i1;}    //registering if output does not equal input
}

for ( i1 = 0; i1 < 20 ;++ i1 )
{   
if (dif[i1]==1 && outarray[i1]==1 && numberofcontrols!=1 ) {cont[i1]=0; gatecount=gatecount+1;flip[gatecount]=i1;for (i3 = 0; i3 < 20 ;++ i3) {control[gatecount][i3]=cont[i3];}}
else if (dif[i1]==1 && outarray[i1]==1 && numberofcontrols==1 ) {repeat =1;}
