...
//=======================================================================
  ...
  for (row=0; row < 1048576; ++row) {
		// Generate output array based on some equations
		// TODO: How dow we know that this is unitary?  Maybe add a test...

		outarray[0]=!array[row][1] && array[row][14] || array[row][7]; //cout << array[row][1] << array[row][14] << array[row][7] << outarray[0]<<"\n";
		outarray[1]=array[row][5] || array[row][17] || array[row][4]; //cout << array[row][5] << array[row][17] << array[row][4] << outarray[1]<<"\n";;
		outarray[2]=!array[row][5] && array[row][6] || array[row][11]; //cout << outarray[2]<<"\n";;
		outarray[3]=array[row][9] && !array[row][3];
		outarray[4]=array[row][1] || array[row][7] || array[row][4] && array[row][5] || array[row][18] || array[row][4];
		outarray[5]=array[row][9] && !array[row][3] && !array[row][2] || array[row][14];
		outarray[6]=!array[row][3] && array[row][17] || array[row][11];
		outarray[7]=array[row][10] || !array[row][7] || array[row][4] && array[row][15] || array[row][8] || array[row][4];
		outarray[8]=array[row][6];
		outarray[9]=array[row][1] || array[row][7] || array[row][4] && !array[row][15] || array[row][9] || array[row][4];
		outarray[10]=!array[row][3] && array[row][17] || array[row][6] && !array[row][3] || array[row][18] || array[row][0];
		outarray[11]=array[row][9] && !array[row][3] || array[row][18] || array[row][0];
		outarray[12]=array[row][14] || !array[row][0] || array[row][6] && !array[row][3] || array[row][18] || array[row][0];
		outarray[13]=array[row][1] && !array[row][3] || array[row][18];
		outarray[14]=array[row][6] && !array[row][9] || array[row][18] || array[row][10];
		outarray[15]=array[row][14] || !array[row][6] || array[row][6] && !array[row][3] || array[row][18] || array[row][0];
		outarray[16]=array[row][1] && !array[row][13] || array[row][18] || array[row][0];
		outarray[17]=array[row][1] || !array[row][6] || array[row][6] && !array[row][3] || array[row][18] || array[row][0];
		outarray[18]=array[row][1] || !array[row][7];
		outarray[19]=array[row][4] || array[row][7] || array[row][4] && array[row][15] || array[row][19] || array[row][4];



		// Apply list of gates synthesized up to this point to new input term
		for ( i1 = 0; i1 < 20 ;++ i1 )
		{
				 for (i4 = 0; i4 < gatecount ;++ i4 )
				 {
						 // Does all control bits match current term.
						 for (i5 = 0; i5 < 20 ;++ i5 ) {
							if (control[i4][i5]==1 && outarray[i5]==0)
								break;
							}    
							
						 // i5 == 20, then all of them matched, so flip it (CNOT).
						 if (i5==20 && outarray[i1]==0)
							outarray[i1]=1;
						 else if (i5==20 && outarray[i1]==1)
							outarray[i1]=0;
				 }
		}
		...
		for ( i1 = 0; i1 < 20 ;++ i1 ) {
			dif[i1]=0;    
			//registering all control lines
			if (outarray [i1]==1) 
				cont [i1]=1; 
			else 
				cont [i1]=0; 
			
			//registering where output does not equal input
			if (outarray [i1] != array[row][i1])
				dif[i1]=1;    
		}
		...
		// Determine new gate
		for ( i1 = 0; i1 < 20 ;++ i1 ) {
			// Flip the gate if bits are different
			if (dif[i1]==1 && outarray[i1]==1) {	// in = 0, out = 1
				cont[i1]=0;
				gatecount=gatecount+1;
				flip[gatecount]=i1;									// 0 -> 1 (no more control line)
				
				// Copy updated control to new gate
				for (i3 = 0; i3 < 20 ;++ i3) {
					control[gatecount][i3]=cont[i3];
				}
			}            
			else if (dif[i1]==1 && outarray[i1]==0) {
				gatecount=gatecount+1;
