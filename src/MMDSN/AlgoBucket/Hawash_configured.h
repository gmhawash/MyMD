...
namespace AlgoBucket {
  namespace Hawash_configured {
    ...
    public:
      ...
      {
        int requestedBits;
        cout << "Enter number of most signifigant bits:";
        cin>> requestedBits;
        Input (nBits, requestedBits);
      }


      Input (ULONGLONG nBits, int requestedBits) : Permutation(nBits)
      {
        SeqName = "Hawash_configured";
        Name = SeqName;
        ...
        for each (vector<ULONGLONG>* a in m_inArray)
          m_nPermutations *= a->size();  //m_nPermutations will conatin number of permutations (factorial)
    ...
    public:
      ...
      STRINGVECTOR generateVector(int bits)
      {
        char buffer[20];
        STRINGVECTOR theVector;
